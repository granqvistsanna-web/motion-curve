...
function NumberInput({ label, value, onChange, min = 1, max = 60, suffix = "min" }: NumberInputProps) {
    return (
        <div className="flex items-center justify-between h-8">
            <label className="text-[11px] text-theme-text-secondary">{label}</label>
            <div className="flex items-center">
                <button
                    onClick={() => onChange(Math.max(min, value - 1))}
                    className="icon-btn w-6 h-6 flex items-center justify-center rounded-full text-theme-text-muted transition-all active:scale-90"
                >
                    <svg width="8" height="8" viewBox="0 0 10 10" fill="none">
                        <path d="M2.5 5H7.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                    </svg>
                </button>
                <span className="w-6 text-center text-[11px] font-semibold text-theme-text tabular-nums">
                    {value}
                </span>
                <button
                ...
                {suffix && <span className="text-[9px] text-theme-text-muted ml-0.5">{suffix}</span>}
            </div>
        </div>
    )
}
...
export function Settings({ settings, onUpdate, onClose, timerStatus, timeRemaining, theme, onToggleTheme }: SettingsProps) {
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            ...
            {/* Utility links */}
            <div className="mt-auto pt-3 flex gap-2 border-t border-theme-border">
                <a
                    href={WIKIPEDIA_URL}
                    target="_blank"
                ...
                >
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" className="text-theme-text-muted group-hover:text-theme-text-secondary transition-colors">
