        ...
        <div className="flex items-center justify-between h-8">
            ...
            <div className="flex items-center" role="group" aria-labelledby={`${id}-label`}>
                <button
                    ...
                    disabled={value <= min}
                    className="w-6 h-6 flex items-center justify-center rounded-full bg-theme-hover-bg text-theme-text-secondary hover:bg-theme-border hover:text-accent transition-all active:scale-90 disabled:opacity-40"
                >
        ...
        <div className="flex items-center justify-between h-8">
            ...
            >
                <span
                    ...
                    style={{
                        backgroundColor: "var(--color-toggle-knob)",
...
export function Settings({ settings, onUpdate, onClose, timerStatus, timeRemaining }: SettingsProps) {
    ...
    return (
        <div className="flex flex-col h-full animate-fade-in">
            ...
            {/* Utility links */}
            <nav className="mt-auto pt-3 flex gap-2 border-t" style={{ borderColor: "var(--color-divider)" }} aria-label="Help and support">
                <a
                    href={WIKIPEDIA_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Learn more about the Pomodoro technique (opens in new tab)"
                    className="flex-1 flex flex-col items-center gap-1.5 py-2.5 rounded-lg transition-all duration-200 group active:scale-[0.97] hover:bg-theme-border hover:scale-[1.02]"
                >
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true" className="text-theme-text-muted group-hover:text-accent transition-colors duration-200">
                    ...
                    <span className="text-[9px] font-medium text-theme-text-muted group-hover:text-theme-text transition-colors duration-200">
                        Learn more
                    </span>
                </a>
                <a
